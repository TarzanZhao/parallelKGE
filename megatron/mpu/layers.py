...
class ParallelEmbedding(torch.nn.Module):
    ...
    def __init__(self, num_embeddings, embedding_dim,
        ...
        self.embeddings = nn.Embedding(self.num_embeddings, self.embedding_dim_per_partition, sparse = True)
        #??? It seems that the embedding is not shared among models. Data parallel group should have the same embedding.
        # And initialize.
        _initialize_affine_weight(
            self.embeddings.weight.data, self.num_embeddings, self.embedding_dim,
            self.embedding_dim_per_partition, 1, init_method,
            stride=1, return_master_weight=False)

    def forward(self, input_):
        # input_parallel = copy_to_model_parallel_region(input_)
        output_parallel = self.embeddings(input_)
